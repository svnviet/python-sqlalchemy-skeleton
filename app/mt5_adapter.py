...
class MT5Gateway:
        ...
        *,
        ...
        if not mt5.initialize():
            ...
            if not exe or not exe.exists():
                err = mt5.last_error()
                raise TradingError(
                    f"MT5 initialize failed and no valid path provided. last_error={err}"
                )
            try:
            ...
            if not mt5.initialize(path=str(exe)):
                raise TradingError(
                    f"MT5 initialize(path=...) failed: {mt5.last_error()}"
                )
        # optional login
    ...
    def market_order(self, req: OrderRequestDTO) -> OrderResultDTO:
        ...
        request = dict(
            ...
            symbol=req.symbol,
            type=(
                mt5.ORDER_TYPE_BUY if req.side == TradeSide.BUY else mt5.ORDER_TYPE_SELL
            ),
        ...
        for o in os_ or []:
            out.append(
                PendingOrderDTO(
                    ticket=o.ticket,
                    symbol=o.symbol,
                    kind=kind_rev.get(o.type, OrderKind.MARKET),
    ...
    def modify_position_sltp(
        ...
        pos = next(
            (p for p in mt5.positions_get() or [] if p.ticket == position_ticket), None
        )
        if not pos:
